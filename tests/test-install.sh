...
assert_jq "$HOME_ONE/.claude/settings.json" '[.hooks.PreToolUse[]? | select(.matcher == "Bash") | .hooks[]? | select(.command == "python3 ~/.claude/hooks/scripts/command_guard.py")] | length == 1' "fresh install has one Bash command_guard hook"
...
assert_jq "$HOME_TWO/.claude/settings.json" '[.hooks[][]?.hooks[]?.command] | map(select(contains("command_guard.py"))) | length == 3' "migration adds command_guard entries once per active matcher"
