...
5. **Check hooks** — verify files in `~/.claude/hooks/`:
   - Scripts: `secret_guard.py`, `large_file_policy.py`, `command_guard.py`, `block-large-files.sh` compatibility wrapper, `block-no-verify.sh`, `warn-debug-code.sh`, `warn-secrets.sh`, `remind-compact.sh`
...
7. **Summary** — output a table:
   ...
   Agents:     4/4 — OK
   Skills:     11/11 — OK
   Hooks:      17/17 files, shell scripts executable — OK
