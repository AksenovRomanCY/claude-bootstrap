...
5. **Check hooks** — verify files in `~/.claude/hooks/`:
   - Scripts: `secret_guard.py`, `large_file_policy.py`, `command_guard.py`, `post_write_warnings.py`, `block-large-files.sh` compatibility wrapper, `block-no-verify.sh`, `warn-debug-code.sh` compatibility wrapper, `warn-secrets.sh` compatibility wrapper, `remind-compact.sh`
   - Guard modules: `guard/__init__.py`, `guard/context.py`, `guard/decisions.py`, `guard/filesystem.py`, `guard/git.py`, `guard/infrastructure.py`, `guard/rules.py`, `guard/secrets.py`, `guard/shell.py`
