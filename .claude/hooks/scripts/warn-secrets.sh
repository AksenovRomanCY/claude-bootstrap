...
if echo "$CONTENT" | grep -qiE '(api_key|apikey|api_secret|secret_key|access_token|auth_token|private_key)\s*[:=]\s*["\x27][a-zA-Z0-9_\-/+]{20,}["\x27]'; then
...
if echo "$CONTENT" | grep -qE '(ghp_[a-zA-Z0-9]{36}|glpat-[a-zA-Z0-9\-]{20,})'; then
